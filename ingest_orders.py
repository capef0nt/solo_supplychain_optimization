...
# Step 4: Get all order IDs from API
...
# Step 5: Fetch full order details
...
# Step 6: Main ingestion function
def ingest_orders():
    ...
    for order_id in all_order_ids:
        # Check if this order already exists in MongoDB
        if collection.count_documents({"_id": order_id}, limit=1) == 0:
            raw_order = fetch_order_details(order_id, session)
            if raw_order:
                collection.update_one({"_id": order_id}, {"$set": raw_order}, upsert=True)
                new_orders_count += 1

    print(f"New orders inserted into MongoDB: {new_orders_count}")
